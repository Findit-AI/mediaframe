...
```toml
mediaframe = { version = "0.7", features = ["yuv-planar", "rgb"] }
```

Or take everything via the umbrella:

```toml
mediaframe = { version = "0.7", features = ["frame"] }
```
...
mediaframe = { version = "0.7", default-features = false }

# no_std + alloc — adds Vec-using helpers and tests.
mediaframe = { version = "0.7", default-features = false, features = ["alloc"] }
```
